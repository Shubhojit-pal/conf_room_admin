...
((SELECT id FROM rooms WHERE name = 'Conference Room C'), (SELECT id FROM users WHERE email = 'carol@company.com'), 'Team Standup', 'Daily engineering standup', NOW() + INTERVAL '1 day', NOW() + INTERVAL '1 day' + INTERVAL '30 minutes', 'confirmed', NULL, NULL),
((SELECT id FROM rooms WHERE name = 'Collaboration Space D'), (SELECT id FROM users WHERE email = 'alice@company.com'), 'Brainstorming Session', 'New product ideas', NOW() + INTERVAL '5 days', NOW() + INTERVAL '5 days' + INTERVAL '2 hours', 'confirmed', NULL, NULL),
((SELECT id FROM rooms WHERE name = 'Executive Suite'), (SELECT id FROM users WHERE email = 'david@company.com'), 'Board Meeting', 'Monthly board review', NOW() + INTERVAL '7 days', NOW() + INTERVAL '7 days' + INTERVAL '3 hours', 'confirmed', NULL, NULL),
